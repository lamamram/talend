...
package formation_s25.api_multithreading_0_1;
...
/**
 * Job: api_multithreading Purpose: extraire toutes les données d'un endpoint REST en multithreadant les appels<br>
 * Description: alimenter la table person avec les données de l'api gorest.co.in <br>
...
public void tLoop_1Process(final java.util.Map<String, Object> globalMap) throws TalendException {
 ...
 final boolean execStat = this.execStat;
	 ...
	 */
				...
				TalendThreadPool mtp_tJava_1 = new TalendThreadPool(Runtime.getRuntime().availableProcessors() / 2);
...
for(int looptLoop_1 =1; looptLoop_1<=60; looptLoop_1=looptLoop_1+1){
...
public static class usersStruct implements routines.system.IPersistableRow<usersStruct> {
    final static byte[] commonByteArrayLock_FORMATION_S25_api_multithreading = new byte[0];
    static byte[] commonByteArray_FORMATION_S25_api_multithreading = new byte[0];
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_FORMATION_S25_api_multithreading.length) {
				if(length < 1024 && commonByteArray_FORMATION_S25_api_multithreading.length == 0) {
   					commonByteArray_FORMATION_S25_api_multithreading = new byte[1024];
				} else {
   					commonByteArray_FORMATION_S25_api_multithreading = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_FORMATION_S25_api_multithreading, 0, length);
			strReturn = new String(commonByteArray_FORMATION_S25_api_multithreading, 0, length, utf8Charset);
		}
		return strReturn;
	}
	...
	private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException{
		...
		if (length == -1) {
			...
			unmarshaller.readFully(commonByteArray_FORMATION_S25_api_multithreading, 0, length);
			strReturn = new String(commonByteArray_FORMATION_S25_api_multithreading, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_FORMATION_S25_api_multithreading) {
...
usersStruct users_save = null;
//the aggregate variable
usersStruct users_aggregate = null;
...
try {
	...
	if (!inOSGi) {
		...
		webClient_tRESTClient_1.header("User-Agent", "talend v8.0.1");
...
if(row2 != null) { 
	...
	boolean isMatchDocRowtXMLMap_1_TXMLMAP_OUT = false;
				...
				absolutePathMappings_tXMLMap_1_TXMLMAP_OUT[1] = "row2.body:/root/root/status";
				relativePathMappings_tXMLMap_1_TXMLMAP_OUT[1] = "status";
				
				absolutePathMappings_tXMLMap_1_TXMLMAP_OUT[2] = "row2.body:/root/root/email";
				relativePathMappings_tXMLMap_1_TXMLMAP_OUT[2] = "email";
...
users_tmp = new usersStruct();
users_tmp.last_name = treeNodeAPI_tXMLMap_1_TXMLMAP_OUT.get_String("row2.body:/root/root/name");
users_tmp.first_name = "";
users_tmp.email = treeNodeAPI_tXMLMap_1_TXMLMAP_OUT.get_String("row2.body:/root/root/email");
users_tmp.active = Convert.toBoolean(treeNodeAPI_tXMLMap_1_TXMLMAP_OUT.get_String("row2.body:/root/root/status"),"active") ;allOutsForAggregate_tXMLMap_1.add(users_tmp);
 ...
 */
	...
	users = null;
	if(row_out_tXMLMap_1_TXMLMAP_IN!=null && row_out_tXMLMap_1_TXMLMAP_IN instanceof usersStruct) {
		users = (usersStruct)row_out_tXMLMap_1_TXMLMAP_IN;		
	}
...
String[] row_tBufferOutput_1=new String[]{"","","","",};		
	    if(users.last_name != null){
	            ...
	            row_tBufferOutput_1[0] = users.last_name;
	    ...
	    if(users.first_name != null){
	            ...
	            row_tBufferOutput_1[1] = users.first_name;
	    ...
	    if(users.email != null){
	            ...
	            row_tBufferOutput_1[2] = users.email;
	    ...
	    if(users.active != null){
	            ...
	            row_tBufferOutput_1[3] = String.valueOf(users.active);
...
int count_tDBOutput_1=0;
				...
				String insert_tDBOutput_1 = "INSERT INTO `" + "person" + "` (`last_name`,`first_name`,`email`,`active`,`slug`) VALUES (?,?,?,?,?)";
...
String[] row_tBufferInput_1 = new String[4];
for (int n = 0; n < globalBuffer.size(); n++)
{
	row_tBufferInput_1 = (String[])globalBuffer.get(n);	
	if(0 < row_tBufferInput_1.length){
	
		input_row.last_name = row_tBufferInput_1[0];	
	
	}
	
	else{
		input_row.last_name = null;
	}	
	if(1 < row_tBufferInput_1.length){
	
		input_row.first_name = row_tBufferInput_1[1];	
	
	}
	
	else{
		input_row.first_name = null;
	}	
	if(2 < row_tBufferInput_1.length){
	
		input_row.email = row_tBufferInput_1[2];	
	
	}
	
	else{
		input_row.email = null;
	}	
	if(3 < row_tBufferInput_1.length){
	
		input_row.active = ParserUtils.parseTo_Boolean(row_tBufferInput_1[3]);
...
String[] elements = {input_row.last_name};
...
} else {pstmt_tDBOutput_1.setString(1, output_row.last_name);
}

                            if(output_row.first_name == null) {
pstmt_tDBOutput_1.setNull(2, java.sql.Types.VARCHAR);
} else {pstmt_tDBOutput_1.setString(2, output_row.first_name);
}

                            if(output_row.email == null) {
pstmt_tDBOutput_1.setNull(3, java.sql.Types.VARCHAR);
} else {pstmt_tDBOutput_1.setString(3, output_row.email);
}

                            if(output_row.active == null) {
pstmt_tDBOutput_1.setNull(4, java.sql.Types.BOOLEAN);
} else {pstmt_tDBOutput_1.setBoolean(4, output_row.active);
}

                            if(output_row.slug == null) {
pstmt_tDBOutput_1.setNull(5, java.sql.Types.VARCHAR);
} else {pstmt_tDBOutput_1.setString(5, output_row.slug);
}
...
public void tDBConnection_1Process(final java.util.Map<String, Object> globalMap) throws TalendException {
 ...
 final boolean execStat = this.execStat;
	...
	currentComponent="tDBConnection_1";
        ...
        String properties_tDBConnection_1 = context.db_out_AdditionalParams + "&allowMultiQueries=true";
...
query_tDBRow_1 = "SET FOREIGN_KEY_CHECKS = 0;TRUNCATE TABLE person";
 ...
 */
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = api_multithreading.class.getClassLoader().getResourceAsStream("formation_s25/api_multithreading_0_1/contexts/" + contextStr + ".properties");
            if (inContext == null) {
                inContext = api_multithreading.class.getClassLoader().getResourceAsStream("config/contexts/" + contextStr + ".properties");
            }
            if (inContext != null) {
                try {
        ...
        if (parentContextMap != null && !parentContextMap.isEmpty()) {if (parentContextMap.containsKey("db_out_AdditionalParams")) {
...
e_tPostjob_1.printStackTrace();
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : api_multithreading");
        }
...
/************************************************************************************************
 *     135631 characters generated by Talend Open Studio for Data Integration 
 *     on the 24 juin 2022 à 16:53:42 CEST
 ************************************************************************************************/