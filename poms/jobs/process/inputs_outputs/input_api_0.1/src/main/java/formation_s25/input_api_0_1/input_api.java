...
package formation_s25.input_api_0_1;
...
/**
 * Job: input_api Purpose: présenter les composants d'extraction d'appels api
 * rest<br>
 * Description: <br>
 * 
 ...
 * @status
 */
public class input_api implements TalendJob {
	...
	private final String jobName = "input_api";
				...
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(input_api.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class row4Struct implements routines.system.IPersistableRow<row4Struct> {
		final static byte[] commonByteArrayLock_FORMATION_S25_input_api = new byte[0];
		static byte[] commonByteArray_FORMATION_S25_input_api = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_FORMATION_S25_input_api.length) {
					if (length < 1024 && commonByteArray_FORMATION_S25_input_api.length == 0) {
						commonByteArray_FORMATION_S25_input_api = new byte[1024];
					} else {
						commonByteArray_FORMATION_S25_input_api = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_FORMATION_S25_input_api, 0, length);
				strReturn = new String(commonByteArray_FORMATION_S25_input_api, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_FORMATION_S25_input_api, 0, length);
				strReturn = new String(commonByteArray_FORMATION_S25_input_api, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_FORMATION_S25_input_api) {
	...
	public void tRESTClient_3Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				util_tLogRow_2.addRow(new String[] { "id", "name", "email", "gender", "status", });
...
// expected response body
				...
				try {
					...
					// set request path
					webClient_tRESTClient_3.path("users");
					...
					webClient_tRESTClient_3.header("User-Agent", "talend v8.0.1");
					...
					webClient_tRESTClient_3.query("page", "1");
					...
					try {
						...
						if (status_tRESTClient_3 != 304 && status_tRESTClient_3 >= 300
								&& responseClass_tRESTClient_3 != javax.ws.rs.core.Response.class) {
							throw org.apache.cxf.jaxrs.utils.ExceptionUtils.toWebApplicationException(
									(javax.ws.rs.core.Response) responseObjBase_tRESTClient_3);
						}
						...
						for (int i_tExtractJSONFields_1 = 0; isStructError_tExtractJSONFields_1
							...
							if (!isStructError_tExtractJSONFields_1) {
								...
								try {
									...
									jsonPath_tExtractJSONFields_1 = "gender";
									compiledJsonPath_tExtractJSONFields_1 = jsonPathCache_tExtractJSONFields_1
											.getCompiledJsonPath(jsonPath_tExtractJSONFields_1);
									...
									try {
										...
										row4.gender = value_tExtractJSONFields_1 == null ?
									...
									try {
										...
										row4.status = value_tExtractJSONFields_1 == null ?
...
///////////////////////		
								...
								if (row4.name != null) { //
									row_tLogRow_2[1] = String.valueOf(row4.name);

								} //

								if (row4.email != null) { //
									row_tLogRow_2[2] = String.valueOf(row4.email);

								} //

								if (row4.gender != null) { //
									row_tLogRow_2[3] = String.valueOf(row4.gender);

								} //

								if (row4.status != null) { //
									row_tLogRow_2[4] = String.valueOf(row4.status);
...
///////////////////////    			
	...
	public static void main(String[] args) {
		final input_api input_apiClass = new input_api();

		int exitCode = input_apiClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = input_api.class.getClassLoader()
					.getResourceAsStream("formation_s25/input_api_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = input_api.class.getClassLoader()
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : input_api");
		}
...
/************************************************************************************************
 * 67804 characters generated by Talend Open Studio for Data Integration on the
 * 23 juin 2022 à 14:16:43 CEST
 ************************************************************************************************/